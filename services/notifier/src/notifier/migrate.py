"""Schema migrations. Run in the stack as `python -m notifier.migrate` (ADR-0006).

Tests use the same functions, so the migrations they check are exactly
the ones the one-shot `notifier-migrate` container applies.
"""
...
def alembic_config(database_url: str) -> Config:
    config = Config()
    config.set_main_option("script_location", "notifier:migrations")
